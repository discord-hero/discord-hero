...
def main(test, **kwargs):
    ...
    if database_type != "sqlite":
        ...
        os.environ["DB_PASSWORD"] = prompt(
            "DB password", value_proc=str, hide_input=True
        )

    cache_type = os.getenv("CACHE_TYPE")
    if os.getenv("CACHE_TYPE") != "simple":
        os.environ["CACHE_HOST"] = prompt(
            "Cache host", value_proc=str, default="localhost"
        )
        os.environ["CACHE_PORT"] = prompt(
            "Cache port",
            value_proc=str,
    ...
    if not os.getenv("BOT_TOKEN"):
        if sys.platform in ("win32", "cygwin", "darwin"):
            if confirm(
                "You need a bot token for your bot. Do you want to create a "
    ...
    if database_type == "sqlite":
        if (
            test and not os.path.isfile(os.path.join(hero.ROOT_DIR, "test_db.sqlite3"))
        ) or (
            not test and not os.path.isfile(os.path.join(hero.ROOT_DIR, "db.sqlite3"))
        ):
    ...
    with open(
        os.path.join(hero.ROOT_DIR, "local_extensions.txt")
    ) as local_extensions_file:
    ...
    try:
        ...
        print(
            style(
                "Error: Database hasn't been initialized yet; use `hero dbinit`",
                fg="red",
            ),
            file=sys.stderr,
        )
    ...
    if created:
        ...
        settings.description = prompt(
            "Short description of your bot", value_proc=str, default=""
        )
        settings.save()
    ...
    with hero.Core(
        config=config,
        settings=settings,
        name=os.getenv("NAMESPACE", "default"),
        loop=loop,
    ) as core:
        core.run()
...
def database_initialization(test, *, from_main=False, **kwargs):
    if not from_main:
        ...
        database_type = os.getenv("DB_TYPE")
        if database_type != "sqlite":
            os.environ["DB_HOST"] = prompt(
                "DB host", value_proc=str, default="localhost"
            )
            os.environ["DB_PORT"] = prompt(
                "DB port",
                value_proc=str,
    ...
    try:
        ...
        try:
            ...
            management.call_command(
                "migrate", "hero", interactive=False, run_syncdb=True
            )
