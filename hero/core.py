...
class Core(commands.Bot):
    ...
    def __init__(self, config, settings, name="default", loop=None):
        ...
        super(Core, self).__init__(
            command_prefix=when_mentioned_or(*self.get_prefixes()),
            loop=loop,
        ...
        user_agent = (
            "discord-hero (https://github.com/discord-hero/discord-hero {0}) "
            "Python/{1} aiohttp/{2} discord.py/{3}"
        )
        self.http.user_agent = user_agent.format(
            hero.__version__,
    ...
    async def on_ready(self):
        ...
        print(
            "\n{} active extensions".format(len(self.__extensions) - 1)
        )  # we don't count essentials
    ...
    def _resolve_groups(self, cog_or_command):
        if isinstance(cog_or_command, hero.Cog):
            for _, member in inspect.getmembers(
                cog_or_command, lambda _member: isinstance(_member, commands.Command)
            ):
            ...
            if "_" in cog_or_command.name:
                ...
                if group_name == "":
                    if entire_group == "" and "__" not in cog_or_command.name:
                        ...
                        raise ValueError(
                            "command {} has two or more consecutive underscores "
                ...
                if group_name in self.all_commands:
                    if not isinstance(self.all_commands[group_name], commands.Group):
                        raise CommandConflict(
                            "cannot group command {0} under {1} because {1} is already a "
                    ...
                    group_command = self.group(
                        name=entire_group, invoke_without_command=True, help=group_help
                    )(groupcmd)
        ...
        if name not in self.config.disabled_extensions:
            if name not in self.__extensions:
                self.__extensions[name] = Extension(
                    name, Extensions.get_extension_module(name, local=_local)
                )
            ...
            if hasattr(cog_module, "setup"):
                ...
                cog_classes = inspect.getmembers(
                    cog_module,
                    lambda member: isinstance(member, type)
                    and issubclass(member, hero.Cog)
                    and member is not hero.Cog,
                )
                for _, _Cog in cog_classes:
                    if _Cog is None:
                        raise ImportError(
                            f"The {name} extension's cog module didn't have "
    ...
    def sync_db(self, *extension_names):
        ...
        print(
            f"Synchronizing database with models from {', '.join(extension_names)}...",
            end=" ",
        )
        ...
        try:
            management.call_command(
                "makemigrations", *extension_names, interactive=False
            )
            management.call_command(
                "makemigrations", *extension_names, interactive=False, merge=True
            )
            for extension_name in extension_names:
                try:
                    management.call_command(
                        "migrate", extension_name, interactive=False
                    )
                except management.CommandError:
                    management.call_command(
                        "migrate", extension_name, interactive=False, run_syncdb=True
                    )
    ...
    async def wait_for_response(
        self, ctx, message_check=None, timeout=60, force_response=True
    ) -> str:
        def response_check(message):
            is_response = (
                ctx.message.author == message.author
                and ctx.message.channel == message.channel
            )
            return is_response and (
                message_check(message) if callable(message_check) else True
            )

        try:
            response = await self.wait_for(
                "message", check=response_check, timeout=timeout
            )
    ...
    async def wait_for_choice(
        self, ctx, choices, timeout=60, force_response=True
    ) -> int:
        ...
        choice = await self.wait_for_response(
            ctx, message_check=choice_check, timeout=timeout
        )
    ...
    async def on_error(self, event_method, *args, **kwargs):
        ...
        if isinstance(value, UserDoesNotExist):
            ...
            if send_message:
                ...
                prefix = self.default_prefix
                register_message = await self.send_gdpr(
                    error_user,
                    author=user,
                ...
                if hero.TEST:
                    ...
                    print(
                        style(
                            f"DEBUG: User {error_user} is not registered in the database; origin: "
                            f"on_{event_method} triggered by user {user} in {where}",
                            fg="bright_blue",
                        )
                    )
            return
    ...
    async def on_command_error(self, ctx: commands.Context, error):
        ...
        if isinstance(error, (UserDoesNotExist, InactiveUser)):
            ...
            if not inactive and register_message is not None:
                saved_user = User(user.id)
                saved_user.is_active = False
                register_message: discord.Message = await self.db.wrap_message(
                    register_message
                )
                saved_user.register_message = register_message
                await saved_user.async_save()
    ...
    async def report_error(self, ctx, error: BaseException, messageable=None):
        if messageable is not None:
            await messageable.send(
                f"An error occured while running the command **{ctx.command}**:"
            )
    ...
    async def send_gdpr(
        ...
        if author is None or author.id == user.id:
            if inactive:
                ...
                try:
                    ...
                    try:
                        await fallback_channel.send(
                            message_text.format(user.mention, prefix),
                ...
                try:
                    register_message = await user.send(
                        message_text.format(user.name, prefix)
                    )
                except discord.Forbidden:
                    register_message = await fallback_channel.send(
                        message_text.format(user.mention, prefix),
            ...
            if inactive:
                ...
                if fallback_channel is not None:
                    try:
                        await fallback_channel.send(
                            message_text.format(author.mention, str(user), prefix),
                            delete_after=delete_after or 60,
                        )
                    except discord.Forbidden:
                        try:
                            await author.send(
                                message_text.format(author.name, str(user), prefix)
                            )
                        except discord.Forbidden:
                            pass
                else:
                ...
                if fallback_channel is not None:
                    try:
                        register_message = await fallback_channel.send(
                            message_text.format(author.mention, user.mention, prefix),
                            delete_after=delete_after or 180,
                        )
                    except discord.Forbidden:
                        try:
                            register_message = await author.send(
                                message_text.format(author.name, str(user), prefix)
                            )
                        except discord.Forbidden:
                            pass
    ...
    def run(self, reconnect=True):
        ...
        try:
            self.loop.run_until_complete(
                self.start(self.config.bot_token, bot=True, reconnect=reconnect)
            )
