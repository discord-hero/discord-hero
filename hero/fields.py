...
from django.db.models.fields.related_descriptors import (
    ManyToManyDescriptor as _ManyToManyDescriptor,
            ...
            if event_loop.is_running():
                if self.field.is_cached(instance):
                    ...
                    return maybe_coroutine(
                        super(ForeignKeyDeferredAttribute, self).__get__,
                        instance,
                        cls=cls,
                    )
                else:
                    return async_using_db(
                        super(ForeignKeyDeferredAttribute, self).__get__
                    )(instance, cls=cls)
            return super(ForeignKeyDeferredAttribute, self).__get__(instance, cls=cls)
        ...
        try:
            ...
            if event_loop.is_running():
                if self.field.is_cached(instance):
                    ...
                    # if accessed from an async context to make behavior more consistent
                    return maybe_coroutine(
                        super(ForwardManyToOneDescriptor, self).__get__,
                        instance,
                        cls=cls,
                    )
                else:
...
class CharField(_CharField):
    def __init__(self, *args, **kwargs):
        if not kwargs.get("max_length"):
            cls = self.__class__
            raise TypeError(
                f"missing required keyword argument 'max_length' in {cls.__name__}"
            )
...
class EmojiField(DiscordField):
    ...
    def contribute_to_class(self, cls, name, private_only=False, **kwargs):
        super(EmojiField, self).contribute_to_class(
...
class ManyEmojisField(ManyDiscordField):
    ...
    def contribute_to_class(self, cls, name, private_only=False, **kwargs):
        super(ManyEmojisField, self).contribute_to_class(
            cls, name, private_only=private_only, **kwargs
        )
        if self.reverse_cascade:
            pre_delete.connect(self._reverse_cascade, sender=cls)
