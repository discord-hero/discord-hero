...
class Extension:
    ...
    def config_cls(self) -> ExtensionConfig:
        config_class = inspect.getmembers(
            self._module,
    ...
    def _controller_cls(self):
        try:
            controller_module = importlib.import_module(
                f"extensions.{self.name}.controller"
            )
        except ImportError:
            try:
                controller_module = importlib.import_module(
                    f"hero.extensions.{self.name}.controller"
                )
            except ImportError:
                return None

        controller_class = inspect.getmembers(
            controller_module,
            lambda member: isinstance(member, type)
    ...
    def _settings_model(self):
        try:
            ...
            try:
                models_module = importlib.import_module(
                    f"hero.extensions.{self.name}.models"
                )
            except ImportError:
                return None
        ...
        settings_model = inspect.getmembers(
            models_module,
            lambda member: isinstance(member, type)
...
class Extensions(dict):
    ...
    def load(self):
        ...
        if _local_extensions:
            ...
            self._local_extensions = []
        _gen = (
            (_name, Extension(_name, self.get_extension_module(_name, local=False)))
            for _name in self._extensions
        )
        _local_gen = (
            (_name, Extension(_name, self.get_extension_module(_name, local=True)))
    ...
    def get_extension_module(cls, name: str, local: bool):
        ...
        if local:
            spec = spec_from_file_location(
                f"extensions.{name}",
                os.path.join(hero.ROOT_DIR, "extensions", name, "__init__.py"),
            )
        else:
            spec = spec_from_file_location(
                f"hero.extensions.{name}",
                os.path.join(hero.LIB_ROOT_DIR, "extensions", name, "__init__.py"),
            )
...
def get_extension_config(extension_name, local=False):
    ...
    try:
        ...
        raise ConfigurationError(
            f"extension {extension_name} doesn't have a hero.ExtensionConfig subclass"
        )
